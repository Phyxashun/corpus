...
describe('Configuration Logic', () => {
  ...
  test('getDefaultConfig should correctly parse the real config.toml', async () => {
    ...
    const staticExcludes = [
      ...
      'node_modules/**/*', 'ALL/**/*', '**/.env', '*/ALL/**/*', '*/ALL_REBUILT/**/*',
    ];
    const expectedRuntimePath = path.join(process.cwd(), 'runtime.config.json');

    expect(config.exclude).toEqual([...staticExcludes, expectedRuntimePath]);
  ...
  test('assignKey leaves target unchanged when the source value is undefined', async () => {
    const config = await getDefaultConfig();
    const originalOutputDir = config.outputDir;

    // Save a PARTIAL config so some keys are missing from the saved JSON
    await saveConfigToDisk({ ...config, theme: 'light' } as any);
    await Bun.write(RUNTIME_CONFIG_PATH, JSON.stringify({ theme: 'light' })); // no outputDir key

    const reloaded = await loadConfig();
    expect(reloaded.theme).toBe('light');
    expect(reloaded.outputDir).toBe(originalOutputDir); // untouched — proves the `if` false-branch ran
  });
});
